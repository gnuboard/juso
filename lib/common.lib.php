...
function get_paging($write_pages, $cur_page, $total_page)
{
    $str = '';
    if ($cur_page > 1) {
        $str .= '<a href="#" onclick="search_call(1); return false;" class="pg_page pg_start">처음</a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<a href="#" onclick="search_call('.($start_page-1).'); return false;" class="pg_page pg_prev">이전</a>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<a href="#" onclick="search_call('.$k.'); return false;" class="pg_page">'.$k.'<span class="sir_sr">페이지</span></a>'.PHP_EOL;
            else
                $str .= '<span class="sir_sr">열린</span><strong class="pg_current">'.$k.'</strong><span class="sir_sr">페이지</span>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<a href="#" onclick="search_call('.($end_page+1).'); return false;" class="pg_page pg_next">다음</a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="#" onclick="search_call('.$total_page.'); return false;" class="pg_page pg_end">맨끝</a>'.PHP_EOL;
    }
